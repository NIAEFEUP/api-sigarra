...
function parseMeals (meals) {
    ...
    meals.forEach((meal, index) => {
        meals[index] = meals[index].replace(
            /(?<!\s(à|a|ante|após|até|com|conforme|contra|consoante|de|desde|durante|em|excepto|entre|mediante|para|por|salvo|sem|segundo|sob|sobre|trás|c\/)) \n[A-Z]/g,
            "~$&"
        );
...
module.exports = {
    ...
    oldReadMeals: function(data) {
        let menu = data.text;
        // let meals_regex = new RegExp("[A-Z]");
        let dates_regex = /\d+ a \d+/g;
        let soups_regex = /Sopa(.+?)Carne/gms;
        let meats_regex = /Carne(.+?)Pescado/gms;
        let fish_regex = /Pescado(.+?)Vegetariano/gms;
        let veggie_regex = /Vegetariano(.+?)NOTAS/gms;
        // let newline_regex  = /\n/g;
        let dates = menu.match(dates_regex);
        if(dates === undefined){
